...
export const createPresignurl: any = asyncHandler(
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (role == "hospital") {
        await axios.post(`${process.env.HOSPITAL_SERVICE_URL}/hospital/${id}`, { imageUrl: uniqueKey }, {
          headers: {
      ...
      if (role == "user") {
        await axios.post(`${process.env.USER_SERVICE_URL}/users/${id}`, { imageUrl: uniqueKey }, {
          headers: {
            Authorization: req.headers.authorization,
          },
        });
      ...
      if (role == "doctor") {
        await axios.post(`${process.env.DOCTOR_SERVICE_URL}/doctor/${id}`, { imageUrl: uniqueKey }, {
          headers: {
            Authorization: req.headers.authorization,
          },
        });
      ...
      if (role == "staff") {
        await axios.post(`${process.env.STAFF_SERVICE_URL}/staff/${id}`, { imageUrl: uniqueKey }, {
          headers: {
            Authorization: req.headers.authorization,
          },
        });
      ...
      if (role == "ad") {
        await axios.post(`${process.env.AD_SERVICE_URL}/ad/${id}`, { imageUrl: uniqueKey }, {
          headers: {
            Authorization: req.headers.authorization,
          },
        });
